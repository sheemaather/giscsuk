...
const SECTIONS = {
  ...
  publications: {
    label: "Publications",
    listFields: ["title", "type", "published_on"],
    formFields: [
      { name: "title", label: "Title", type: "text", required: true },
      { name: "type", label: "Type", type: "select", options: ["text", "image", "pdf"] },
      { name: "body", label: "Body / Description", type: "textarea" },
      { name: "published_on", label: "Published On", type: "date", required: true },
      { name: "file", label: "File (image or PDF)", type: "file" }
    ]
  },
  gallery: {
    label: "Gallery",
...
function showForm(key, existing = null) {
  ...
  const fieldsHtml = cfg.formFields.map(f => {
    const value = existing ? (existing[f.name] ?? "") : "";
    if (f.type === "textarea") {
